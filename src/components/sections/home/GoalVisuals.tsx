...
/**
 * One mark per financial goal — Phase 3.
 *
 * The goals section had no visual language at all: six identical cards with
 * a lucide glyph in a grey box, which is exactly the "flat icon-only
 * treatment" the phase brief rules out. These are the contextual treatment
 * that replaces it — a small drawn subject per goal, in that goal's own
 * accent, sitting inside the planning panel.
 *
 ...
 * ── Construction ────────────────────────────────────────────────────────
 *
 * One viewBox, one plate, one stroke helper, one node. Colour comes entirely
 * from the tone channel (`--tone`, see globals.css), so each mark renders in
 * its goal's accent — and in that accent's independently tuned dark
 * instance — without naming a colour. The only animation is the stroke
 * drawing itself once on entry, which is disabled under reduced motion; the
 * finished drawing is the base state, so nothing here is ever waiting on an
...
interface PlateProps extends MarkProps {
  label: string;
  markId: string;
  children: ReactNode;
}

/** Shared frame: the tone field, the dot texture, the caption. */
function Plate({ label, markId, className, children }: PlateProps) {
  return (
    <figure
      className={cn(
        'relative flex h-full w-full flex-col justify-between overflow-hidden rounded-surface',
        'border border-tone/25 bg-tone-tint p-4',
        className
      )}
    >
      <span
        aria-hidden="true"
        className="pointer-events-none absolute -right-8 -top-10 h-36 w-36 rounded-pill bg-tone opacity-20 blur-3xl"
      />
      ...
      >
        <defs>
          <linearGradient id={markId + '-area'} x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="rgb(var(--tone))" stopOpacity="0.32" />
            <stop offset="100%" stopColor="rgb(var(--tone))" stopOpacity="0" />
          </linearGradient>
          <pattern id={markId + '-grid'} width="13" height="13" patternUnits="userSpaceOnUse">
            <circle cx="1.3" cy="1.3" r="1" fill="rgb(var(--tone))" opacity="0.18" />
          </pattern>
        </defs>
...
function Node({ cx, cy, lead = false }: { cx: number; cy: number; lead?: boolean }) {
  return (
    <circle
      cx={cx}
      cy={cy}
      r={lead ? 6 : 4}
      fill={lead ? 'rgb(var(--tone))' : 'rgb(var(--color-surface))'}
      stroke="rgb(var(--tone))"
      strokeWidth={2.25}
    />
  );
...
export function GrowMark({ className }: MarkProps) {
  return (
    <Plate label="Left alone, for years" markId="goal-grow" className={className}>
      <path d={GROW_PATH + ' L 214 124 L 26 124 Z'} fill="url(#goal-grow-area)" />
      ...
      {[
        ...
        <rect key={x} x={x - 5} y={y} width={10} height={124 - y} rx={3} fill="rgb(var(--tone))" fillOpacity={0.16 + index * 0.05} />
      ))}
      <Stroke d={GROW_PATH} />
      <Node cx={26} cy={112} />
      <Node cx={128} cy={62} />
      <Node cx={214} cy={26} lead />
    </Plate>
  );
}
...
export function ProtectMark({ className }: MarkProps) {
  return (
    <Plate label="What sits behind the cover" markId="goal-protect" className={className}>
      <path d={SHIELD} fill="rgb(var(--tone))" fillOpacity={0.12} />
      <Stroke d={SHIELD} />
...
export function HomeMark({ className }: MarkProps) {
  return (
    <Plate label="The place, and the repayment" markId="goal-home" className={className}>
      <path d="M 82 72 L 82 126 L 158 126 L 158 72 Z" fill="rgb(var(--tone))" fillOpacity={0.14} />
      <path d="M 64 74 L 120 32 L 176 74 Z" fill="rgb(var(--tone))" fillOpacity={0.2} />
...
export function TaxMark({ className }: MarkProps) {
  return (
    <Plate label="What a deduction takes out" markId="goal-tax" className={className}>
      <path d="M 72 20 L 146 20 L 170 44 L 170 132 L 72 132 Z" fill="rgb(var(--tone))" fillOpacity={0.12} />
      <Stroke d="M 72 20 L 146 20 L 170 44 L 170 132 L 72 132 Z" />
...
export function EducationMark({ className }: MarkProps) {
  return (
    <Plate label="A date you already know" markId="goal-education" className={className}>
      ...
      {[
        ...
        <rect key={x} x={x - 16} y={y} width={32} height={130 - y} rx={4} fill="rgb(var(--tone))" fillOpacity={0.12 + index * 0.06} />
      ))}
...
export function RetirementMark({ className }: MarkProps) {
  return (
    <Plate label="The climb, then the level part" markId="goal-retire" className={className}>
      <path d={RETIRE_PATH + ' L 216 128 L 24 128 Z'} fill="url(#goal-retire-area)" />
